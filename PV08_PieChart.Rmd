...
```{r}
gl.pop <- gl.read.dart(filename = "Data/Report_DPoc22-7156_SNP_mapping_2.csv", covfilename = "Data/Pverrucosa_DartLIMS_pop.csv")

gl.pop <- gl.drop.pop(gl.pop, 
                  pop.list =c("Chinaman"))
...
pver_gen.pop <- gl2gi(gl3.pop, probar = FALSE, verbose = NULL)
pver_gen.pop

is.genind(pver_gen.pop)
```
...
isPoly(pver_gen.pop) %>% summary

#remove loci that are not polymorphic 

poly_loci = names(which(isPoly(pver_gen.pop) == TRUE))
pver_gen.pop = pver_gen.pop[loc = poly_loci]
isPoly(pver_gen.pop) %>% summary
```
...
# Add individual IDs
qmatrix$Ind = indNames(pver_gen.pop)

#Add site IDs
qmatrix$Site = pver_gen.pop$pop
...
pver.pie %>% arrange(variable)


ggplot(pver.pie, aes(x = "", y = mean, fill = factor(variable))) +
...
save(pver.pie, file = "pver.pie.RData")
